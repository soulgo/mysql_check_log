...
# 导入所需库
import logging
import logging.handlers # 不再需要
import os # 不再需要 os 来创建目录
...
def index():
    ...
    server_options = []
    try:
        # 直接从 config.LOG_CONFIG['servers'] (列表) 构建服务器选项
...
def api_scan():
    ...
    try:
        ...
        if not server_list or not isinstance(server_list, list):
             ...
             response_data = {'status': 'error', 'error': '服务器配置错误'}
             status_code = 500
        # 根据是否有 server_id 执行不同逻辑
        elif server_id_to_scan is not None:
            # 扫描指定服务器
