...
### 5.1 功能概述
系统提供了完整的定期报表功能，支持生成日报、周报和月报。每种报表都包含详细的操作统计信息，帮助管理员了解数据库使用情况和潜在风险。

### 5.2 报表类型
* **日报**：统计最近24小时内的数据库操作情况
* **周报**：统计最近7天内的数据库操作情况
* **月报**：统计最近30天内的数据库操作情况
...
2. **风险等级统计**：
   ...
   * 中危操作数量统计
   * 低危操作数量统计
   * 使用不同颜色标识不同风险等级（红色-高危、黄色-中危、绿色-低危）
...
### 5.4 使用方法
1. 在系统界面中点击"报表导出"选项卡
2. 选择需要生成的报表类型（日报/周报/月报）
3. 点击对应的"生成报表"按钮
4. 系统将弹出报表预览窗口，显示完整的统计信息
5. 可以通过窗口右上角的关闭按钮关闭报表预览

### 5.5 注意事项
* 报表生成基于系统当前的风险规则配置
* 报表统计包含所有已记录的操作，不受写入过滤规则影响
* 时间统计基于操作发生的实际时间，非记录入库时间
* 报表数据实时生成，反映生成时刻的最新统计结果
